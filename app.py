...
if st.session_state.results:
    ...
    if consolidated_path.exists():
        with open(consolidated_path, "rb") as f:
            csv_cols[0].download_button("📄 Consolidated CSV", f, file_name="if_consolidated.csv", use_container_width=True)

    if summary_path.exists():
        with open(summary_path, "rb") as f:
            csv_cols[1].download_button("📄 Summary CSV", f, file_name="if_summary_by_person.csv", use_container_width=True)

    if master_path.exists():
        with open(master_path, "rb") as f:
            csv_cols[2].download_button("📄 Master CSV", f, file_name="if_master.csv", use_container_width=True)

    # Overlays ZIP — only shown if checkbox was ticked at run time
    overlays_dir = output_dir / "overlays"
    if (
        st.session_state.results["save_overlays"]
        and overlays_dir.exists()
        and any(overlays_dir.iterdir())
    ):
        overlay_files = [f for f in overlays_dir.rglob("*") if f.is_file()]
        zip_buf = io.BytesIO()
        with zipfile.ZipFile(zip_buf, "w", zipfile.ZIP_DEFLATED) as zf:
            for f in overlay_files:
                zf.write(f, arcname=f.relative_to(overlays_dir))
        zip_buf.seek(0)
        st.download_button(
            f"🖼️ Download Overlays ZIP ({len(overlay_files)} files)",
            zip_buf,
    ...
    # Landmarks ZIP — only shown if checkbox was ticked at run time
    landmarks_dir = output_dir / "landmarks"
    if (
        st.session_state.results["save_landmarks"]
        and landmarks_dir.exists()
        and any(landmarks_dir.iterdir())
    ):
        landmark_files = [f for f in landmarks_dir.rglob("*") if f.is_file()]
        zip_buf = io.BytesIO()
        with zipfile.ZipFile(zip_buf, "w", zipfile.ZIP_DEFLATED) as zf:
            for f in landmark_files:
                zf.write(f, arcname=f.relative_to(landmarks_dir))
        zip_buf.seek(0)
        st.download_button(
            f"📍 Download Landmarks ZIP ({len(landmark_files)} files)",
            zip_buf,
